...
\*   \*\*Opt-In:\*\* The bot is inactive until an administrator runs the `c.setup` command.

\*   \*\*Opt-Out:\*\* Administrators can run `c.teardown` at any time to immediately stop all relays and \*\*permanently delete\*\* the server's configuration and webhook data from our database.
